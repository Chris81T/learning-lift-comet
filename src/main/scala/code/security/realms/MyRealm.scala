...
object UserManagement {
  ...
  val users = User("master", "master", "admin") :: 
	  	      User("batman", "batman", "admin") :: 
	  	      User("ironman", "ironman", "admin") :: Nil

  /**
   * Simply define hardcoded some followers..
   */	  	      
  val followers : Map[User, List[User]] = Map(users(0) -> (users(1) :: users(2) :: Nil),
		  									  users(1) -> (users(2) :: Nil),
		  									  users(2) -> (users(1) :: Nil))  
